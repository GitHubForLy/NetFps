...
namespace Game.UI.Panel
{
    public class RoomListPanel :PanelBase
    {
        ...
        [SerializeField]
        private GameObject HisCombatContent;
        [SerializeField]
        private GameObject HisCombatPrefab;
        ...
        public override void OnInit(params object[] paramaters)
        {
            ...
            TcpRequest.Instance.DoRequest<RoomInfos>(null, TcpFlags.TcpGetRoomList,res=> 
            {
                foreach (var rom in res.Infos)
                {
        ...
        {
            TcpRequest.Instance.DoRequest<Respone<UserInfo>>(NetManager.Instance.LoginAccount, TcpFlags.TcpGetUserInfo, res =>
            {
                if(res.IsSuccess)
                {
                    var user = res.Data;
                    Inst("用户昵称", user.UserName);
        ...
        {
            if(action==TcpFlags.TcpBdCreateRoom)
            {
                RoomInfo data =(RoomInfo)formatter(typeof(RoomInfo));
                    InstantiateRoomTitle(data);
            }
            else if(action==TcpFlags.TcpBdLeaveRoom)
            {
                RoomInfo data = (RoomInfo)formatter(typeof(RoomInfo));
            ...
            else if(action==TcpFlags.TcpBdJoinRoom || action == TcpFlags.TcpRoomFight /*|| action == TcpFlags.TcpGameFinished*/)
            {
                RoomInfo data = (RoomInfo)formatter(typeof(RoomInfo));
                SetRoomInfo(GetRoomTitleForId(data.RoomId), data);
            }
        ...
        public void JoinRoom()
        {
            if (selected == null)
                return;
            if (selected.State != RoomState.RoomWaiting)
                return;
        ...
        private void DoJoinRoom(string password)
        {
            ...
            TcpRequest.Instance.DoRequest<Respone<RoomInfo>>((selected.RoomId, password), TcpFlags.TcpJoinRoom, res =>
            {
                lod.Close();
                if (res.IsSuccess)
                {
        ...
        private void GetHisCombats()
        {
            TcpRequest.Instance.DoRequest<CombatInfos>(null,TcpFlags.TcpGetHisCombat, res =>
            {
                foreach(var cm in res.Infos)
                {
                    var obj= Instantiate(HisCombatPrefab, HisCombatContent.transform);
                    obj.GetComponent<HisCard>().SetInfo(cm);
                }
            });
        }
