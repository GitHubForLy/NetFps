...
namespace Game.UI
{
    public class PanelManager : MonoBehaviour
    {
        ...
        private Dictionary<PanelBase,GameObject> m_OpenPanels=new Dictionary<PanelBase,GameObject>();
        //private List<GameObject> m_OpenPanels = new List<GameObject>();

        /// <summary>
        /// 当前打开的最上层Panel
        /// </summary>
        public PanelBase TopOpenPanel => m_OpenPanels.Last().Key;
...
//#else
            if (ritbtn)
            {
                if (m_OpenPanels.Count > 0)
                    m_OpenPanels.Last().Key.OnEscape();
                else
                    OnEscape?.Invoke();
            }
//#endif
        ...
        {
            return m_OpenPanels.Any(m => m.Key is T);
        }
        ...
        public T OpenPanelUnique<T>(params object[] paramaters) where T : PanelBase
        {
            if(IsOpen<T>())
            {           
                var panel =m_OpenPanels.First(m => m.Key is T);
                panel.Value.transform.parent.SetAsLastSibling();
                return (T)panel.Key;
            }
            return (T)OpenPanel(typeof(T), paramaters);
        }
        ...
        public PanelBase OpenPanel(string PanelPath,params object[] paramaters)
        {
            ...
            m_OpenPanels.Add(panelbase, gamePanel);
            Cursor.visible = true;
        ...
        {
            if (panel.IsDestroyed)   //该物体已经被销毁了
            {
                if (m_OpenPanels.ContainsKey(panel))
        ...
        public void CloseAll()
        {
            ...
            {
                panel = m_OpenPanels.ElementAt(i).Key;
                panel.OnClosed();
